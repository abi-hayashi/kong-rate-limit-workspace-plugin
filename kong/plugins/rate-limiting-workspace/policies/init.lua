local policy_cluster = require "kong.plugins.rate-limiting-workspace.policies.cluster"
...
local EXPIRATION = require "kong.plugins.rate-limiting-workspace.expiration"
...
local function sync_to_redis(premature, conf)
  ...
  if not red then
    kong.log.err("[rate-limiting-workspace] failed to connect to Redis: ", err)
  ...
  if err then
    kong.log.err("[rate-limiting-workspace] failed to commit increment pipeline in Redis: ", err)
    clear_local_counter(db_key)
    return
  end
  ...
  if not ok then
    kong.log.err("[rate-limiting-workspace] failed to set Redis keepalive: ", err)
    clear_local_counter(db_key)
    return
  end
...
local function rate_limited_sync(conf, sync_func)
  local cache_key = conf.__key__ or conf.__plugin_id or "rate-limiting-workspace"
  local redis_config = get_redis_configuration(conf)
